...
func TestHandleAmpersandSign(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			token, err := tokenizer.handleAmpersandSign(1)
...
func TestHandleAmpersandSignErr(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := NewTokenizer(test.input).handleAmpersandSign(1)
