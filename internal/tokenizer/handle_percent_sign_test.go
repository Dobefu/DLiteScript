...
func TestHandlePercentSign(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:  "equal sign after percent sign",
			input: "%=",
			expected: token.NewToken(
				"%=",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			token, err := tokenizer.handlePercentSign(1)
...
func TestHandlePercentSignErr(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := NewTokenizer(test.input).handlePercentSign(1)
