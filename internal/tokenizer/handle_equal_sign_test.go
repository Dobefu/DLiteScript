...
func TestHandleEqualSign(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			token, err := tokenizer.handleEqualSign(1)
...
func TestHandleEqualSignErr(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := NewTokenizer(test.input).handleEqualSign(1)
